...
#define CDV_CAPTURE_PREFIX @"cdv_capture_"
...
static inline BOOL IsIPhone5(void) {
...
- (CDVPluginResult*)processImage:(UIImage*)image type:(NSString*)mimeType forCallbackId:(NSString*)callbackId
{
    ...
    do {
        ...
        filePath = [NSString stringWithFormat:@"%@/%@%03d.jpg", docsPath, CDV_CAPTURE_PREFIX, i++];
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary*)info
{
    ...
    if (image != nil) {
        ...
        if (moviePath) {
            ...
            [library writeVideoAtPathToSavedPhotosAlbum:[NSURL URLWithString:moviePath] completionBlock:^(NSURL *assetURL, NSError *error){

                CDVPluginResult *basicResult = nil;

                if(error == nil) {
                    NSDictionary* fileDict = [self getMediaDictionaryFromPath:[assetURL absoluteString] ofType:nil];
                    NSArray *fileArray = [NSArray arrayWithObject:fileDict];

                    __block CDVPluginResult *albumResult = nil;

                    [library assetForURL:assetURL resultBlock:^(ALAsset *asset) {
                        NSString* originalFileName = [[asset defaultRepresentation] filename];
                        NSMutableDictionary *newDict = [NSMutableDictionary dictionaryWithDictionary:fileDict];
                        [newDict setObject:originalFileName forKey:@"name"];
                        albumResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsArray:[NSArray arrayWithObject:newDict]];
                        [self.commandDelegate sendPluginResult:albumResult callbackId:callbackId];
                        pickerController = nil;
                    } failureBlock:^(NSError *error) {
                        albumResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsArray:fileArray];
                        [self.commandDelegate sendPluginResult:albumResult callbackId:callbackId];
                        pickerController = nil;
                    }];

                } else {
                    basicResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageToErrorObject:CAPTURE_INTERNAL_ERR];
                }

                if (basicResult) {
                    [self.commandDelegate sendPluginResult:basicResult callbackId:callbackId];
                    pickerController = nil;
                }
            }];
        }
...
- (void)addSkipBackupAttributeToItemAtURL:(NSURL *)itemURL {
    NSError *error = nil;
    BOOL success = [itemURL setResourceValue:[NSNumber numberWithBool:YES] forKey: NSURLIsExcludedFromBackupKey error:&error];
    if (!success) {
        NSLog(@"Error excluding %@ from backup %@",[itemURL lastPathComponent], error);
    }
}

- (void)addSkipBackupAttributeToItemAtPath:(NSString *)itemPath {
    [self addSkipBackupAttributeToItemAtURL:[NSURL fileURLWithPath:itemPath]];
}
...
- (void)cleanup:(CDVInvokedUrlCommand*)command
{
    ...
    NSDirectoryEnumerator* directoryEnumerator = [fileMgr enumeratorAtPath:documentDirectoryPath];
    NSString* fileName = nil;
    BOOL result;

    while ((fileName = [directoryEnumerator nextObject])) {
        // only delete the files we created
        if (![fileName hasPrefix:CDV_CAPTURE_PREFIX]) {
            continue;
        }
...
- (void)viewDidLoad
{
    ...
    do {
        ...
        filePath = [NSString stringWithFormat:@"%@/%@%03d.wav", docsPath, CDV_CAPTURE_PREFIX, i++];
    } while ([fileMgr fileExistsAtPath:filePath]);

    NSURL* fileURL = [NSURL fileURLWithPath:filePath isDirectory:NO];

    [self.captureCommand addSkipBackupAttributeToItemAtURL:fileURL];
