...
- (void)takePicture:(CDVInvokedUrlCommand*)command
{
    ...
    if (!hasCamera) {
        NSLog(@"Camera.getPicture: source type %lu not available.", sourceType);
    ...
    if ([mediaType isEqualToString:(NSString*)kUTTypeImage]) {
        if (cameraPicker.returnType == DestinationTypeNativeUri) {
            ...
            if (cameraPicker.saveToPhotoAlbum) {
                ...
                [library writeImageToSavedPhotosAlbum:returnedImage.CGImage orientation:(ALAssetOrientation)(returnedImage.imageOrientation) completionBlock:^(NSURL *assetURL, NSError *error){

                    CDVPluginResult* albumResult = nil;

                    if(error == nil) {
                        albumResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:[assetURL absoluteString]];
                    } else {
                        albumResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_IO_EXCEPTION messageAsString:[error localizedDescription]];
                    }

                    if (albumResult) {
                        [self.commandDelegate sendPluginResult:albumResult callbackId:cameraPicker.callbackId];
                    }

                }];
            }

            else if (cameraPicker.returnType == DestinationTypeFileUri) {
...
- (void)imagePickerControllerReturnImageResult
{
    ...
    if (self.pickerController.saveToPhotoAlbum) {
        ...
        [library writeImageDataToSavedPhotosAlbum:self.data metadata:self.metadata completionBlock:^(NSURL *assetURL, NSError *error){
            ...
            if (albumResult) {
                [self.commandDelegate sendPluginResult:albumResult callbackId:self.pickerController.callbackId];
            }
    ...
    else if (self.pickerController.returnType == DestinationTypeFileUri) {
        // write to temp directory and return URI
        // get the temp directory path
        NSString* docsPath = [NSTemporaryDirectory()stringByStandardizingPath];
        NSError* err = nil;
